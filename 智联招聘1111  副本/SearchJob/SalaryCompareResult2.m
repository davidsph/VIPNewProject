...
@implementation SalaryCompareResult2
...
-(void)readXMLString
{
    ...
    if ([[root children] count]==2) {
        ...
        salaryCompareResult.experience =self.experience;
        
        salaryCompareResult.querySalary =self.querySalary;//查询的薪水
        salaryCompareResult.compareSalary =self.compareSalary;//比较的薪水
...
-(void)selectedRowIn:(SelectQueryConditons *)sel
{
    ...
    switch (selectedRow) {
        ...
        case 7://第一个选择条件
            ...
            switch (num) {
                case 0:
                    /*queryString 和 compareQueryStringID  为请求接口的参数,compareQueryString 为后面需要显示的值，此处变量偏多，一定要区分开每个变量代表的含义*/
